...
> **⚠️ 关于登录方式的说明**
>
> 知乎已对二维码 / 账号密码 / 短信 / 微信等接口加入反爬签名校验（`x-zse-96` 等），
> 非浏览器客户端无法再直接调用这些登录接口。因此本插件改用「复用真实浏览器会话」的方式登录，
> 与 Obsidian 的知乎插件思路一致。推荐使用下面两种方式，旧的二维码 / 密码方式已默认隐藏。

### 从浏览器导入登录（推荐）

选择「从浏览器导入登录」后，插件会用一个**独立的浏览器配置目录**启动本机的 Chrome / Edge，
并打开知乎登录页。在弹出的浏览器窗口里用知乎 APP 扫码（或任意方式）登录成功后，
插件会通过 Chrome DevTools 协议直接读取该会话的 Cookie（包含 `z_c0`、`__zse_ck` 等），
自动完成登录。整个过程不会触碰你日常使用的浏览器配置，登录状态会保存在独立目录中以便下次复用。

- 需要本机已安装 Chrome 或 Edge。
- 之所以不直接读取浏览器 Cookie 文件：Chrome / Edge 127+ 对磁盘上的 Cookie 启用了
  App-Bound 加密（`v20`），无法离线解密；通过调试协议从运行中的浏览器读取则不受影响。

### 手动粘贴 Cookie 登录

如果没有安装 Chrome / Edge，可选择「手动粘贴 Cookie 登录」：在浏览器登录知乎后按 `F12`
打开开发者工具，切到 Network 面板刷新页面，点击任意请求，复制请求头里的完整 `Cookie` 值，
粘贴到输入框即可。

### 二维码 / 账号密码（已停用）

> 以下方式因知乎反爬升级已无法工作，代码保留仅作参考。
