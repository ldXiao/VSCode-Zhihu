...
export class PublishService {
    ...
    async publish(textEdtior: vscode.TextEditor, edit: vscode.TextEditorEdit) {
        ...
        if (pubLater) {
            ...
            let h = parseInt(timeStr.replace(ClockReg, "$1")),
                m = parseInt(timeStr.replace(ClockReg, "$2")),
                aOrPm = timeStr.replace(ClockReg, "$3");
            if (!timeStr) return;
    ...
    private async _selectColumn(): Promise<IColumn | undefined> {
        ...
        return vscode.window
            .showQuickPick<vscode.QuickPickItem & { value: IColumn }>(
                [{ label: "不发布到专栏", value: undefined }].concat(
                    columns.map((c) => ({ label: c.title, value: c }))
                ),
                {
                    ignoreFocusOut: true,
                }
    ...
    public async postArticle(
        ...
        let patchResp = await sendRequest({
            ...
            body: {
                ...
                title: title,
                titleImage,
                isTitleImageFullScreen: vscode.workspace
                    .getConfiguration("zhihu")
                    .get(SettingEnum.isTitleImageFullScreen),
            },
            headers: {},
        });
