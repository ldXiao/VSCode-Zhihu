...
export class PasteService {
    ...
    public async uploadImageFromExplorer(uri?: vscode.Uri) {
        const imageUri = await vscode.window
            .showOpenDialog({
                canSelectFiles: true,
    ...
    public async uploadImageFromLink(
        ...
        } else if (picType == "mermaid" && outerPic.test(link)) {
        ...
        if (prefetchBody.upload_token) {
            ...
            zhihu_agent.options.accessKeySecret =
                prefetchBody.upload_token.access_key;
            zhihu_agent.options.stsToken =
                prefetchBody.upload_token.access_token;
                ...
                .then((r) => {
                    if (insert) {
                        this.insertImageLink(
                            `${
                                prefetchBody.upload_file.object_key
                            }${path.extname(link)}`
                        );
                    }
                })
        ...
        return Promise.resolve(
            `${ImageHostAPI}/v2-${hash}${path.extname(link)}`
        );
    }
    /**
    ...
    private saveClipboardImageToFileAndGetPath(
        ...
        if (platform === "win32") {
            ...
            const ascript = childProcess.spawn("osascript", [
                scriptPath,
                imagePath,
            ]);
