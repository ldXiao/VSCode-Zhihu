...
export class HotStoryTreeViewProvider implements vscode.TreeDataProvider<ZhihuTreeItem> {
    ...
    private readonly _onDidChangeTreeData: vscode.EventEmitter<ZhihuTreeItem | undefined> = new vscode.EventEmitter<ZhihuTreeItem | undefined>();
    ...
    getChildren( element?: ZhihuTreeItem ): Thenable<ZhihuTreeItem[]> {
        ...
        if ( element ) {
            return new Promise( async ( resolve, reject ) => {
                const hotStoryAPI = `${HotStoryAPI}/${element.type}?desktop=true`;
                httpClient( hotStoryAPI, { json: true }, ( err, _res, body ) => {
                    const questions: HotStory[] = body.data;
                    const deps: ZhihuTreeItem[] = questions.map( story => {
                        return new ZhihuTreeItem( story.target.title ? story.target.title : "", "", vscode.TreeItemCollapsibleState.None,
                            {
