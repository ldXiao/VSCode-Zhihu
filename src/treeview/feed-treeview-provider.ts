        ...
        if ( element ) {
            if ( element.type == "root" ) {
                ...
                return new Promise( async ( resolve, reject ) => {
                    ...
                    const feedAPI = `${FeedStoryAPI}?page_number=${element.page}&limit=10&action=down`;
                    ...
                    feedResp = feedResp.data.filter( f => {
                        return f.target.type != "feed_advert";
                    } );
                    const deps: FeedTreeItem[] = feedResp.map( feed => {
                        const type = feed.target.type;
